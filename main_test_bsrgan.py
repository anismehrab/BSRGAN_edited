...
#            model.load_state_dict(state_dict, strict=True)
        ...
        for testset_L in testset_Ls:
            ...
            for img in util.get_image_paths(L_path):
                ...
                # --------------------------------
                start = time.time()
                #logger.info("image Low resoluiton size:{}".format(img_L.size()))
                #img_E = scripted_model(img_L)
                img_E = model(img_L)
                #logger.info("image High resoluiton size:{}".format(img_E.size()))
                logger.info("inference time: {}".format(time.time() - start))


                    
                # elif(idx == 2):
                #     testRuntime(modelonnx_path,img_L,img_E)
                # --------------------------------
                # (3) img_E
                # --------------------------------
                img_E = util.tensor2uint(img_E) #return numpy array H*W*C RGB
                #logger.info("save image High resoluiton size:{}".format(np.shape(img_E)))
